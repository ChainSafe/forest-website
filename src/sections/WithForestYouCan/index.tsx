...
export default function WithForestYouCan() {
  return (
    <div className="relative h-full w-full bg-no-repeat bg-cover md:min-h-[70vh] bg-multi-gradient">
      <div className="relative flex flex-col justify-center md:items-center md:py-14 px-8">
          <h2 className='mt-7 text-white text-3xl font-normal'>
            With Forest, you can...
          </h2>
          <div className='grid md:grid-cols-4 py-24'>
          {properties.map((card, index) => (
            <div key={index} className='py-4 flex flex-col justify-start'>
              <div className='rounded-lg h-20 w-20 p-5 bg-gray-800 opacity-40'>
                {card.icon}
              </div>
              <h3 className='mt-2 md:mt-7 text-gray-300 text-xl leading-snug max-w-[80%]'>{card.desc}</h3>
            </div>
          ))}
          </div>
        <h2 className='md:mt-7 md:max-w-xl md:text-center text-white text-2xl'>
