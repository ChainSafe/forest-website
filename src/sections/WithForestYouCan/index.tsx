      ...
      <div className="relative flex flex-col justify-center md:items-center py-14 px-8">
          ...
          {properties.map((card, index) => (
            <div key={index} className='py-4 flex flex-col justify-start'>
              <div className='rounded-lg h-20 w-20 p-5 bg-gray-900 opacity-50'>
                {card.icon}
              </div>
              <h3 className='mt-2 md:mt-7 text-gray-200 text-xl leading-6 font-light max-w-[80%]'>{card.desc}</h3>
            </div>
          ))}
          </div>
